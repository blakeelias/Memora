...
for f in auto_photos:
    if 'jpg' in f: 
        ...
        img = cv2.resize(632, 464)
        cv2.imshow('Is this image too blurry?',img)
        k = cv2.waitKey(33)
        vals = blurry(f)
        if k == ord('y'):    
            plt.scatter(vals[0], vals[1], c=1)
        elif k == ord('n'):  
            plt.scatter(vals[0], vals[1], c=100)
        else:
